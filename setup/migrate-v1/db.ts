...
export async function run(args: string[]): Promise<void> {
  ...
  // `channel_name` was added to upstream NanoClaw mid-1.x; some forks
  // predate it. Probe the schema and adapt the SELECT — when the column
  // is absent, inferChannelType() falls back to the JID prefix path,
  // which still resolves every supported channel.
  const v1Cols = v1Db
    .prepare('PRAGMA table_info(registered_groups)')
    .all() as Array<{ name: string }>;
  const hasChannelName = v1Cols.some((c) => c.name === 'channel_name');
  const v1Groups = v1Db
    .prepare(
      hasChannelName
