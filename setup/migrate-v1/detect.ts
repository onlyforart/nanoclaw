...
export async function run(args: string[]): Promise<void> {
  ...
  for (const candidate of candidates) {
    ...
    // Don't self-match auto-discovered candidates — if the candidate
    // resolves to the v2 checkout we're running inside, skip it. Protects
    // users who cloned v2 into `~/nanoclaw` after deleting v1.
    //
    // When the user explicitly named the path (via --path or
    // NANOCLAW_V1_PATH), trust them: in-place upgrades legitimately have
    // v1-root == v2-root == cwd (the v2 worktree is squash-merged onto
    // the v1 checkout). The looksLikeV1Install check below still gates on
    // package.json version, so a v2 path explicitly named will still be
    // rejected with reason=already_v2 — we don't lose protection.
    if (!explicit && absolute === path.resolve(process.cwd())) continue;
