 ...
 *
 * Honors NANOCLAW_NO_DIAGNOSTICS=1. Telemetry is disabled in this fork:
 * NANOCLAW_PH_KEY is empty by default, so emit() short-circuits before
 * any network call. Set NANOCLAW_PH_KEY in env to opt back in.
 */
import { randomUUID } from 'crypto';
import fs from 'fs';
import path from 'path';

const POSTHOG_KEY = process.env.NANOCLAW_PH_KEY || '';
