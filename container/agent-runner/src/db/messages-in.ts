...
export interface MessageInRow {
  ...
  content: string;
  /**
   * Per-message engine override (e.g. "ollama:qwen3", "anthropic:haiku").
   * NULL = use the container's default provider. The poll-loop passes
   * this to engine-selector via QueryInput for per-message routing.
   */
  model: string | null;
  /**
   * Per-message SDK override for scheduled tasks. NULL/0 = anthropic-api
   * (production default for `kind='task'`); 1 = SDK. The poll-loop reads
   * this for engine routing only — interactive messages (kind='chat') ignore it.
   */
