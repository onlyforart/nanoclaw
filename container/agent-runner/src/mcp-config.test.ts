...
describe('buildSdkMcpServers', () => {
  ...
  it('leaves stdio entries unchanged', () => {
    ...
    expect(mcpServers['domain-tools']).toEqual({
      command: 'node',
      args: ['/workspace/mcp-servers/domain-tools/build/index.js'],
      env: { SOME_VAR: 'value' },
    });
    expect(mcpTools).toEqual(['mcp__domain-tools__check_venue_status']);
  });
  ...
  it('handles mixed HTTP and stdio config', () => {
    ...
    expect(mcpServers['domain-tools']).toHaveProperty('command', 'node');
    expect(mcpServers['domain-tools']).not.toHaveProperty('type');
    ...
    expect(mcpTools).toEqual([
      'mcp__domain-tools__check_venue_status',
      'mcp__eks-kubectl__list_pods',
    ]);
  });
