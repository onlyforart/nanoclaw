...
import { pickRoutingFields } from './poll-loop.js';
...
describe('pickRoutingFields', () => {
  function row(over: Partial<MessageInRow>): MessageInRow {
    return {
      ...
      recurrence: null,
      tries: 0,
      trigger: over.trigger ?? 1,
      platform_id: null,
      channel_type: null,
      thread_id: null,
      content: '{}',
      model: over.model ?? null,
      use_agent_sdk: over.use_agent_sdk ?? null,
    };
  }

  it('returns empty object on no messages', () => {
    expect(pickRoutingFields([])).toEqual({});
  });

  it('returns empty object when primary has no override and is not a task', () => {
    expect(pickRoutingFields([row({ kind: 'chat' })])).toEqual({});
  });

  it('exposes model when primary has an override', () => {
    expect(pickRoutingFields([row({ model: 'ollama:qwen3' })])).toEqual({ model: 'ollama:qwen3' });
  });

  it('flags isScheduledTask when primary kind is task', () => {
    expect(pickRoutingFields([row({ kind: 'task' })])).toEqual({ isScheduledTask: true });
  });

  it('exposes useAgentSdk: true when use_agent_sdk = 1', () => {
    expect(pickRoutingFields([row({ kind: 'task', use_agent_sdk: 1 })])).toEqual({
      isScheduledTask: true,
      useAgentSdk: true,
    });
  });

  it('exposes useAgentSdk: false when use_agent_sdk = 0', () => {
    expect(pickRoutingFields([row({ kind: 'task', use_agent_sdk: 0 })])).toEqual({
      isScheduledTask: true,
      useAgentSdk: false,
    });
  });

  it('omits useAgentSdk entirely when use_agent_sdk is NULL', () => {
    expect(pickRoutingFields([row({ kind: 'task', use_agent_sdk: null })])).toEqual({
      isScheduledTask: true,
    });
  });

  it('picks the highest-seq trigger=1 message as primary', () => {
    const result = pickRoutingFields([
      row({ id: 'a', seq: 1, model: 'ollama:old', trigger: 1 }),
      row({ id: 'b', seq: 5, model: 'ollama:new', trigger: 1 }),
      row({ id: 'c', seq: 10, model: 'should-not-win', trigger: 0 }),
    ]);
    expect(result).toEqual({ model: 'ollama:new' });
  });

  it('falls back to highest-seq message when no trigger=1 row exists', () => {
    const result = pickRoutingFields([
      row({ id: 'a', seq: 1, model: 'ollama:a', trigger: 0 }),
      row({ id: 'b', seq: 5, model: 'ollama:b', trigger: 0 }),
    ]);
    expect(result).toEqual({ model: 'ollama:b' });
  });

  it('combines model + isScheduledTask + useAgentSdk on a single primary task row', () => {
    const result = pickRoutingFields([
      row({ id: 'a', seq: 1, model: 'anthropic:haiku', kind: 'task', use_agent_sdk: 0 }),
    ]);
  ...
  // K.1.f step 9.0 Commit 3 — NANOCLAW_MODEL env var fallback.
  //
  // Per-wiring model overrides flow host → container via env vars set at
  // container spawn (NANOCLAW_MODEL). When a per-message override is
  // present on messages_in, it still wins — env var is the SESSION default
  // for the wiring, not a per-message replacement. Empty string is treated
  // as unset (matches v1 fork's `'' → undefined` convention elsewhere).
  describe('NANOCLAW_MODEL env fallback', () => {
    afterEach(() => {
      delete process.env.NANOCLAW_MODEL;
    });

    it('NM1 — falls back to NANOCLAW_MODEL when primary has no per-message model', () => {
      process.env.NANOCLAW_MODEL = 'ollama:llama3.1';
      expect(pickRoutingFields([row({ kind: 'chat' })])).toEqual({
        model: 'ollama:llama3.1',
      });
    });

    it('NM2 — per-message model wins over NANOCLAW_MODEL env', () => {
      process.env.NANOCLAW_MODEL = 'ollama:llama3.1';
      expect(pickRoutingFields([row({ model: 'anthropic:haiku' })])).toEqual({
        model: 'anthropic:haiku',
      });
    });

    it('NM3 — empty NANOCLAW_MODEL is treated as unset (no fallback applied)', () => {
      process.env.NANOCLAW_MODEL = '';
      expect(pickRoutingFields([row({ kind: 'chat' })])).toEqual({});
    });

    it('NM4 — fallback combines with isScheduledTask / useAgentSdk fields', () => {
      process.env.NANOCLAW_MODEL = 'ollama:qwen3';
      expect(pickRoutingFields([row({ kind: 'task', use_agent_sdk: 1 })])).toEqual({
        model: 'ollama:qwen3',
        isScheduledTask: true,
        useAgentSdk: true,
