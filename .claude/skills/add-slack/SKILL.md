---
name: add-slack
description: Add Slack channel integration via Chat SDK. NOT FOR THIS FORK — see README-extra.md (Socket Mode is mandatory; running this skill or setup/install-slack.sh would replace the @slack/bolt adapter with the webhook-only Chat SDK shim).
---

# Add Slack Channel

> **⚠️ DO NOT RUN THIS SKILL ON THIS FORK.**
>
> This fork mandates Slack Socket Mode (`@slack/bolt`) — see
> `README-extra.md`. The adapter at `src/channels/slack.ts` is the
> fork-only Socket Mode port; replacing it with the upstream webhook
> shim is a production regression. `setup/install-slack.sh` has a
> refusal guard for the same reason.
>
> If you arrive at this skill on a fresh checkout because the Slack
> adapter looks "missing", build from the committed source instead
> (`pnpm install && pnpm run build`) — the adapter IS the committed
> source. Do NOT run this skill or the install script.
