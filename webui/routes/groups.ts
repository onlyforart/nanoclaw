...
export function handleGetGroupTokenUsage(folder: string, days: number = 30): DailyTokenUsageRow[] | null {
  ...
  for (const row of rows) {
    ...
    // Cost: use actual if available, otherwise estimate from pricing table
    // — but only when cache breakdown is present (post-fix data). Pre-fix rows
    //   stored base-only input_tokens with no cache columns, so estimates would
