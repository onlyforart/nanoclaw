...
Add two columns to `registered_groups`:
...
ALTER TABLE registered_groups
  ADD COLUMN threading_mode TEXT NOT NULL DEFAULT 'temporal';
  -- values: 'temporal' | 'thread_aware'
```

- **active** — current behaviour. Messages with the trigger pattern invoke the agent.
- **passive** — messages are captured to the `messages` table but the agent is never invoked. Bot must still be a member of the channel for Slack/Telegram to deliver events. Passive groups are automatically discovered as sanitiser source channels (see Source channel resolution).
- **control** — reserved for future use (channel can issue commands but never receives bot responses).

The `threading_mode` field controls the monitor's clustering strategy for this channel. Default `temporal` (cluster by time/participants/topics). Channels with >5% threading rate should use `thread_aware` (cluster primarily on `thread_ts`). Initially set manually; the monitor auto-updates it based on observed threading rates (see Phase E).
...
The web UI gets per-group dropdowns for both `mode` and `threading_mode`.
...
CREATE TABLE IF NOT EXISTS observed_messages (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  source_chat_jid TEXT,                  -- channel JID (passive); nullable for intake without channel context
  source_message_id TEXT,                -- original Slack/etc message id; nullable for intake
  source_type TEXT NOT NULL DEFAULT 'passive_channel',  -- 'passive_channel' | 'task_intake'
  source_task_id TEXT,                   -- submitting task ID (intake only)
  source_group TEXT,                     -- submitting group (intake only)
  intake_reason TEXT,                    -- why submitted (intake only)
  intake_event_id INTEGER,              -- back-reference to intake.raw event (intake only)
...
-- Dedup for passive-channel observations
...
-- Dedup for intake observations (one observation per intake event)
...
**Job:** read new rows from `messages` for monitored source channels and process `intake.raw` events submitted by other tasks via `submit_to_pipeline` (see section 9), extract structured observations, write the result to `observed_messages.sanitised_json`, publish an `observation.*` event for downstream consumers. Each message (including thread replies) becomes one observation. The sanitiser **walks thread replies**, not just top-level messages — corpus analysis shows ~37% of substantive text in high-threading channels and hidden INC/RITM references live inside threads.
...
**LLM API access:** Layer 2's API calls go through the **credential proxy** (`src/credential-proxy.ts`), the same path used by containers. The host-side LLM client makes HTTP requests to `http://127.0.0.1:{CREDENTIAL_PROXY_PORT}/v1/messages` with placeholder credentials; the proxy injects the real API key. This maintains the invariant that no code outside the credential proxy ever reads API keys from `.env`. Token usage from the API response is captured and logged to `task_run_logs` via `logTaskRun()`, the same path used by container-based tasks.

**Topology:** the pipeline code is the same whether it processes one channel or many. The YAML spec takes a `source_channels` list — a shared instance lists multiple channels, a dedicated instance lists one. Start with a single shared instance; split to per-channel if a channel's volume or schema needs diverge. Intake events (`intake.raw`) are a second input source processed in the same run — the sanitiser claims pending intake events alongside its passive-channel scan and runs them through the same three-layer pipeline.
...
### 9. Pipeline intake mechanism

The passive-channel path (sections 2–5) handles messages that arrive naturally in monitored channels. But a task in any group may encounter content that should be routed through the pipeline — for example, a forwarded email, a pasted log, or an issue discovered during investigation that belongs in the observation flow rather than the current task's scope.

The intake mechanism provides a structured way for any task to submit content to the sanitiser, ensuring it flows through the same three-layer pipeline as passive-channel messages.

#### `submit_to_pipeline` MCP tool

New MCP tool exposed via `ipc-mcp-stdio.ts`:

```ts
submit_to_pipeline(
  raw_text: string,              // the content to sanitise
  source_context: {
    source_type: string,         // 'task' | 'message' | 'external'
    source_group: string,        // group that found this
    source_task_id?: string,     // task that found this (if scheduled task)
    source_channel?: string,     // originating channel JID (if known)
    source_message_id?: string,  // originating message ID (if known)
    reason: string,              // why this was submitted
  },
  dedupe_key?: string            // optional dedup (e.g. derived from source_message_id)
) -> { ok: true, event_id: number } | { ok: false, error: string }
```

The tool validates required fields, logs the submission to `pipeline_intake_log`, then publishes an `intake.raw` event. It does **not** write to `observed_messages` directly — the sanitiser does that when it processes the event, maintaining the invariant that all observations flow through the three-layer pipeline.

The `source_context` is mandatory and fully recorded. Every submission is traceable to the task, group, and reason that produced it.

#### `intake.raw` event type

Published by the `submit_to_pipeline` IPC handler. Payload:
...
```json
{
  ...
  "source_context": {
    ...
    "source_channel": "sl:{channel_id}",
    "source_message_id": "1712345678.123456",
    "reason": "Encountered during cluster investigation"
  }
}
```

The sanitiser's `subscribed_event_types` includes `intake.raw`. When the event lands, `bumpConsumerTaskNextRun` fires, and the sanitiser picks it up on the next scheduler tick (seconds, not waiting for the cron interval). This uses the same latency mitigation mechanism designed for `observation.*` → monitor bumping (section 1).

#### `pipeline_intake_log` table

Separate audit trail for all intake submissions, independent of the events table (which has TTL/expiry) and `observed_messages` (which only exists after sanitisation):

```sql
CREATE TABLE IF NOT EXISTS pipeline_intake_log (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  event_id INTEGER NOT NULL,           -- the intake.raw event ID
  raw_text_hash TEXT NOT NULL,         -- SHA-256 of raw_text (for dedup audit, not the text itself)
  ...
  processed_at TEXT,                   -- set when sanitiser picks it up
  observation_id INTEGER               -- back-reference to observed_messages row once processed
);
```

This gives a complete record of what was submitted, by whom, why, and whether it was processed — regardless of event expiry.

#### Security notes

- The `submit_to_pipeline` tool is subject to `allowed_tools` enforcement like any other MCP tool. Pipeline tasks that should not submit intake content simply don't have it in their tool list.
- Submitted content passes through the full three-layer sanitiser, including PII redaction, truncation, and adversarial detection. The intake path has the same security posture as the passive-channel path.
- The `dedupe_key` prevents the same content from being submitted twice (e.g. if two tasks encounter the same forwarded email). Dedup is via the `intake.raw` event's standard `dedupe_key` mechanism.
...
type: host_pipeline                # maps to execution_mode column (see below)
model: anthropic:haiku             # model for Layer 2 LLM extraction
cron: "*/1 * * * *"
subscribed_event_types:
  - "intake.raw"                   # bumped on intake submission (section 9)
system: |-
...
The sanitiser and responder YAML specs reference `{source_channel}` — a placeholder resolved at loader time. The loader discovers source channels by querying `registered_groups WHERE mode = 'passive'`. This means:

- Registering a group as passive automatically makes it a sanitiser source. No separate config needed.
- The web UI's mode dropdown is the primary interface for adding/removing source channels.
- The responder's `allowed_send_targets` is populated from the same query, restricting replies to passive channels only.
- If no passive groups exist, the sanitiser processes only intake events (no passive-channel scan).

The resolved channel list is logged at startup and visible in the web UI's task detail page (as the effective `source_channels` value). If a group's mode changes while the pipeline is running, the change takes effect on the next sanitiser run (the loader re-queries on each reconciliation cycle, not just at startup).

### Execution mode

The YAML `type` field maps to a new `execution_mode` column on `scheduled_tasks`:

| YAML `type` | `execution_mode` | Behaviour |
|-------------|-------------------|-----------|
| (absent) | `container` (default) | Current behaviour: spawns a container |
| `host_pipeline` | `host_pipeline` | Task scheduler calls host-side code directly; no container |

This is a **separate column** from `context_mode` (which controls session isolation: `group` vs `isolated`). `execution_mode` controls *where* the task runs; `context_mode` controls *how* the container's session state is managed. For `host_pipeline` tasks, `context_mode` is ignored (no container, no session).
...
3. `read_chat_messages` MCP tool
4. `observed_messages` table (skeleton — `sanitised_json` nullable until the sanitiser exists; includes `source_type` and intake-tracking columns)
...
1. **Monitor task with bimodal clustering and classification logic.** The monitor is responsible for correlating individual observations into incidents/issues. Corpus analysis shows clustering cannot rely on `thread_ts` alone — most channels have threading rates ≤2%, and even high-threading channels have problem reports in the main timeline. The monitor must implement two clustering modes, applied per channel (selected via `threading_mode` field on `registered_groups`, auto-updated by the monitor):
   ...
   - **Temporal/topical mode** (low-threading channels): cluster on temporal proximity (30-60 minute windows), participant overlap, `referenced_tickets[]` co-occurrence, and `speech_act` sequences (e.g. `fresh_report` → `status_update` → `fix_announcement`). Must accept single-message observations as valid clusters — roughly half of customer-affecting reports in some channels are 1-3 messages.
