...
The monitor receives structured observations via `consume_events(types=['observation.*'])`. Corpus analysis shows clustering cannot rely on `thread_ts` alone — most channels have threading rates ≤2%.

The monitor implements two clustering modes, selected per channel via a `threading_mode` field on `registered_groups`:

**Thread-aware mode** (`threading_mode: 'thread_aware'`, for channels with >5% threading rate):
- Primary: cluster on `thread_ts`
- Fallback: temporal/participant clustering for main-timeline messages without `thread_ts`

**Temporal/topical mode** (`threading_mode: 'temporal'`, default for most channels):
...
**Mode selection:** The `threading_mode` field is initially set manually (via web UI or IPC) based on known channel characteristics. The monitor auto-updates it: after each batch, it computes the threading rate from the last 7 days of observations for each source channel. If the rate crosses the 5% threshold (in either direction), the monitor publishes a `threading_mode_changed` event and updates `registered_groups` via IPC. This means a channel that starts using threads more (or less) will automatically switch clustering strategies without manual intervention.
...
The approval flow bridges channel reactions (human intent) to the events table (pipeline input). It requires two pieces: reaction capture from the channel, and a host-side bridge that translates reactions into pipeline events.

#### Prerequisites

1. **Reaction capture** — the channel must deliver reaction events to the host. For Slack, this means adding a `reaction_added` event handler in `src/channels/slack.ts` alongside the existing `message` handler. The channel normalizes reactions into a `Reaction` interface:
   ```ts
   interface Reaction {
     emoji: string;           // e.g. 'thumbsup', 'thumbsdown', 'speech_balloon'
     userId: string;          // who reacted
     messageId: string;       // which message they reacted to (Slack ts)
     chatJid: string;         // which channel
     timestamp: string;       // when they reacted
   }
   ```
   The `add-reactions` skill provides WhatsApp reaction capture; Slack needs an equivalent handler.

2. **`onReaction` callback** — a new callback in `src/index.ts` channel options, parallel to the existing `onMessage`:
   ```ts
   const channelOpts = {
     onMessage: (chatJid, msg) => { ... },
     onReaction: (chatJid, reaction) => handleReaction(reaction, deps),
     // ...
   };
   ```
   This is the integration point — the channel delivers normalized reactions, the host dispatches to the reaction bridge.

#### Reaction-to-event bridge

**src/reaction-bridge.ts** — new file:

- `handleReaction(reaction: Reaction, deps: ReactionBridgeDeps): Promise<void>` — main entry point, called from `onReaction`:
  1. Check if `reaction.messageId` matches a pending `proposed_reply` event (by `team_message_id` in payload). If not, ignore (reaction on a non-proposal message).
  2. Dispatch by emoji:
     - `:thumbsup:` → `publishApprovedReply()`
     - `:thumbsdown:` → `publishRejection()`
     - `:speech_balloon:` → `startEditFlow()`
  3. Duplicate reactions (same user, same emoji, same message) are idempotent — the bridge checks event status before acting.

- `findProposedReply(messageId: string): ProposedReplyEvent | null` — queries `events` table for a pending `proposed_reply` event whose payload `team_message_id` matches. Returns null if not found or already processed.

- `publishApprovedReply(event: ProposedReplyEvent, text: string): void` — publishes `approved_reply` event with the proposed text, target channel, idempotency key, and observation IDs from the original proposal.

- `publishRejection(event: ProposedReplyEvent, reason: string): void` — acks the `proposed_reply` event as `done` with note "rejected by human".

- `startEditFlow(event: ProposedReplyEvent, userId: string): void` — publishes an `edit_requested` event with `{ userId, proposedReplyEventId, ttl_seconds: 1800 }` (30-minute window).

#### Edit flow implementation

The `:speech_balloon:` → edit path has a two-step lifecycle:

1. **Reaction received:** The bridge publishes an `edit_requested` event (30-minute TTL). The event payload contains the `userId` and the original `proposed_reply` event ID.

2. **Follow-up message intercepted:** The `onMessage` callback in `src/index.ts` gains an early check — before storing the message and dispatching to the agent:
   ```ts
   // In onMessage, after remote control check, before storeMessage:
   const editEvent = findPendingEditRequest(chatJid, msg.senderId);
   if (editEvent) {
     completeEditFlow(editEvent, msg.text);
     return;  // consume the message — don't store or forward to agent
   }
   ```
   `findPendingEditRequest(chatJid, userId)` queries `events` for a pending `edit_requested` event matching the team channel and user. `completeEditFlow()` publishes `approved_reply` with the message text as the edited reply and acks the `edit_requested` event.

3. **Expiry:** If no follow-up message arrives within the TTL, the `edit_requested` event expires. The original proposal remains pending — the user can still `:thumbsup:` the unedited version or let it expire too.

**Why intercept in `onMessage`:** The edit message is not a command to the agent — it's a reply body destined for another channel. Consuming it early prevents the agent from seeing and responding to it. The `onMessage` check is lightweight (one DB query for pending `edit_requested` events) and only fires in the team channel.

**Integration point:** `src/reaction-bridge.ts` exports both `handleReaction` (for the `onReaction` callback) and `findPendingEditRequest` + `completeEditFlow` (for the `onMessage` early check). The bridge is channel-agnostic — it receives normalized reactions and messages.
...
5. **Edit flow.** The `:speech_balloon:` → edit path uses a two-step event lifecycle: reaction → `edit_requested` event (30-min TTL) → follow-up message intercepted in `onMessage` before agent dispatch → `approved_reply` event published. The message is consumed (not stored or forwarded to the agent) because it's a reply body, not a command. The `onMessage` check is one DB query per message in the team channel, gated on the channel having any pending `edit_requested` events.
...
| src/reaction-bridge.ts | 3 | **new** — reaction → event bridge + edit flow |
| src/reaction-bridge.test.ts | 3 | **new** — tests |
| src/channels/slack.ts | 3 | add `reaction_added` event handler |
| src/index.ts | 3 | add `onReaction` callback + edit flow message interception |
