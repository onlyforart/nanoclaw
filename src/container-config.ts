...
/**
 * Remote (HTTP) MCP server entry. Mirrors v1's `RemoteMcpServerEntry` —
 * fields are kept verbatim per fork-original carry-across, even when
 * dormant on this install (`proxy`/`policies` are dormant until the
 * `mcp-auth-proxy` startup is wired; tracked separately).
 *
 * `toolSchemas` is host-populated at container spawn (via
 * `discoverRemoteToolSchemas`) so the agent-runner sees the resolved
 * schema list without hitting the upstream itself.
 */
...
/**
 * An MCP server entry can be either a stdio server (command-spawned) or
 * a remote server (HTTP-fetched). Discriminate by presence of `url`.
 */
export type McpServerEntry = McpServerConfig | RemoteMcpServerConfig;
...
export interface ContainerConfig {
  ...
  /**
   * Per-agent-group env vars passed verbatim to the container as `-e KEY=VALUE`.
   * Applied after OneCLI's gateway env so they override gateway-managed values.
   * Used for provider-redirection (e.g. `ANTHROPIC_BASE_URL` to a local Ollama).
   */
  env?: Record<string, string>;
  /**
   * Hostnames to make unreachable inside the container by mapping each to
   * 0.0.0.0 via Docker's `--add-host`. Used to block accidental egress to
   * (e.g.) `api.anthropic.com` when the agent group is meant to run locally.
   */
  blockedHosts?: string[];
}
