...
export interface ChannelSetup {
  ...
  /**
   * Called when a reaction event arrives from the platform. Optional —
   * adapters that don't observe reactions (or platforms without them)
   * leave this off. Wired by `buildChannelSetup` to the host's
   * `dispatchReaction` from `src/reaction-handlers.ts`. See §4.6.6 of
   * the v1→v2 migration plan (D1).
   */
  onReaction?(reaction: Reaction): void | Promise<void>;
}

/**
 * A single reaction event observed on a channel adapter.
 *
 * Type-only addition under §4.6.5; the wiring of `ChannelSetup.onReaction`
 * (D1) and the calling of `dispatchReaction` from the host's adapter setup
 * lands in §4.6.6. Plugins register handlers via
 * `host.registerReactionHandler(...)` today; without §4.6.6 the dispatch
 * path is dormant.
 */
...
export interface ChannelAdapter {
  ...
  /**
   * Pipeline reacji-approval helper: read draft text from a PROPOSED REPLY
   * the bot already posted to the platform. Returns null when the message
   * is unavailable (deleted, permissions, etc.). Adapters omit this when
   * the platform has no per-id message fetch. See §4.6.6 (D2).
   */
  fetchMessageText?(platformId: string, messageId: string): Promise<string | null>;

  /**
   * Pipeline F6.2 delivery verification: confirm a reply landed in a thread
   * by listing the thread's replies. Returns null when the platform has no
   * thread-replies endpoint or the call failed. See §4.6.6 (D2).
   */
  fetchThreadReplies?(platformId: string, threadId: string): Promise<Array<{ id: string; text: string | null }> | null>;

  /**
   * Pipeline passive-observation backfill: replay platform history from a
   * cursor. Returns null when the platform has no history API. See §4.6.6
   * (D3).
   */
  backfillFromCursor?(platformId: string, oldestTimestamp: string): Promise<InboundMessage[] | null>;
}
