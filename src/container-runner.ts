...
/**
 * OneCLI spawn gate (K.1.h A.5). Refuses to spawn the container if the
 * gateway can't be applied — operator stance is that an agent must never
 * see raw credentials, so a missing/broken gateway is a hard error rather
 * than a silent degrade-to-direct-egress.
 *
 * Exported (named) so it can be unit-tested with a fake OneCLI client.
 * `onecliClient` is parameterised for the same reason — production calls
 * pass the module-level `onecli`.
 */
export async function applyOneCLIGateway(
  onecliClient: Pick<OneCLI, 'ensureAgent' | 'applyContainerConfig'>,
  args: string[],
  ...
  if (agentIdentifier) {
    await onecliClient.ensureAgent({ name: agentGroup.name, identifier: agentIdentifier });
  }
  const applied = await onecliClient.applyContainerConfig(args, {
    addHostMapping: false,
    agent: agentIdentifier,
  });
  if (!applied) {
    throw new Error(
      `OneCLI gateway not applied for ${containerName} — refusing to spawn ` +
        'container without credential proxy. Check that the OneCLI daemon is ' +
        'running and the agent has secrets assigned (or secret-mode=all).',
    );
  }
...
async function spawnContainer(session: Session): Promise<void> {
  ...
  let containerConfig = readContainerConfig(agentGroup.folder);
  ...
  // Resolve remote MCP servers: rewrite localhost URLs, discover tool schemas,
  // inline skill content. Stdio entries pass through. Persisted back to
  // container.json so composeGroupClaudeMd (called downstream from buildMounts)
  // and the agent-runner read the resolved view from the RO mount.
  const mcpBefore = JSON.stringify(containerConfig.mcpServers);
  containerConfig = await resolveMcpServers(containerConfig);
  if (JSON.stringify(containerConfig.mcpServers) !== mcpBefore) {
  ...
  // K.1.f step 9.0 — per-wiring agent settings. Read the wiring's
  // is_main/model/temperature/max_tool_rounds/timeout_ms/show_thinking
  // override values, fall back to backend-defaults[agent_provider], and
  // inject as NANOCLAW_* env vars into the container.
  const wiringEnv = wiringSettingsToEnv(resolveWiringSettings(session, agentGroup));
...
/**
 * Build the per-agent-group `-e` and `--add-host` flag pairs for a container
 * spawn. Env entries come first, blockedHosts second; both inject 0.0.0.0 for
 * blocked hosts so they are unreachable from inside the container. Caller is
 * responsible for inserting these AFTER OneCLI's gateway args so per-group
 * overrides win on duplicate keys.
 */
export function groupEnvAndBlockedHostsArgs(
  config: Pick<import('./container-config.js').ContainerConfig, 'env' | 'blockedHosts'>,
): string[] {
  const args: string[] = [];
  if (config.env) {
...
async function buildContainerArgs(
  ...
  wiringEnv: Record<string, string> = {},
): Promise<string[]> {
  ...
  // K.1.f step 9.0 — per-wiring agent settings env vars
  // (NANOCLAW_MODEL / NANOCLAW_TEMPERATURE / NANOCLAW_MAX_TOOL_ROUNDS /
  // NANOCLAW_TIMEOUT_MS / NANOCLAW_SHOW_THINKING). Resolved in
  // spawnContainer (wiring override → backend-defaults fallback).
  // NULL fields are skipped — agent-runner falls through to its engine
  // defaults rather than receiving an empty string.
  for (const [key, value] of Object.entries(wiringEnv)) {
    args.push('-e', `${key}=${value}`);
  }

  // OneCLI gateway — injects HTTPS_PROXY + certs so container API calls
  // are routed through the agent vault for credential injection.
  // FATAL on failure (K.1.h A.5): the operator explicitly refuses to let
  // any agent see raw credentials, so a missing/broken gateway must block
  // the spawn rather than silently degrade to direct-egress.
  await applyOneCLIGateway(onecli, args, agentGroup, agentIdentifier, containerName);
  ...
  // effect alongside the host-gateway --add-host above.
  args.push(...groupEnvAndBlockedHostsArgs(containerConfig));
