...
export function insertTask(
  ...
  task: {
    ...
    content: string;
    /** Per-task engine override (e.g. "ollama:qwen3"). NULL = container default. */
    model?: string | null;
    /** Per-task SDK override. NULL/0 = anthropic-api (production default); 1 = SDK. */
    useAgentSdk?: 0 | 1 | null;
  },
): void {
  db.prepare(
    `INSERT INTO messages_in (id, seq, timestamp, status, tries, process_after, recurrence, kind, platform_id, channel_type, thread_id, content, series_id, model, use_agent_sdk)
     VALUES (@id, @seq, datetime('now'), 'pending', 0, @processAfter, @recurrence, 'task', @platformId, @channelType, @threadId, @content, @id, @model, @useAgentSdk)`,
  ).run({
    ...task,
    model: task.model ?? null,
    useAgentSdk: task.useAgentSdk ?? null,
    seq: nextEvenSeq(db),
  });
}
...
export interface RecurringMessage {
  ...
  series_id: string;
  /** Carried over to the next occurrence so engine routing is stable across reruns. */
  model: string | null;
  /** Carried over to the next occurrence — see `model`. */
...
export function insertRecurrence(
  ...
  db.prepare(
    `INSERT INTO messages_in (id, seq, kind, timestamp, status, process_after, recurrence, platform_id, channel_type, thread_id, content, series_id, model, use_agent_sdk)
