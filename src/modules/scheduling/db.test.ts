...
describe('insertTask', () => {
  ...
  it('writes NULL model + use_agent_sdk by default', () => {
    const db = freshDb();
    insertBasicTask(db, 'task-default', null);
    const row = db.prepare('SELECT model, use_agent_sdk FROM messages_in WHERE id = ?').get('task-default') as {
      model: string | null;
  ...
  it('persists model + use_agent_sdk overrides when provided', () => {
    const db = freshDb();
    insertTask(db, {
      id: 'task-ollama',
      processAfter: new Date().toISOString(),
    ...
    const row = db.prepare('SELECT model, use_agent_sdk FROM messages_in WHERE id = ?').get('task-ollama') as {
      model: string | null;
      use_agent_sdk: number | null;
    };
    expect(row.model).toBe('ollama:qwen3');
    expect(row.use_agent_sdk).toBe(0);
    db.close();
  });
});
...
describe('insertRecurrence', () => {
  ...
  it('carries model + use_agent_sdk forward to the next occurrence', () => {
    const db = freshDb();
    insertTask(db, {
      id: 'task-orig',
      processAfter: new Date().toISOString(),
    ...
    const completed = getCompletedRecurring(db);
    expect(completed).toHaveLength(1);
    insertRecurrence(db, completed[0], 'task-next', new Date().toISOString());

    const next = db.prepare('SELECT model, use_agent_sdk FROM messages_in WHERE id = ?').get('task-next') as {
      model: string | null;
      use_agent_sdk: number | null;
    };
    expect(next.model).toBe('anthropic:haiku');
    expect(next.use_agent_sdk).toBe(0);
    db.close();
  });
});
