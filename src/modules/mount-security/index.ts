...
/**
 * Reset the cached allowlist so the next `loadMountAllowlist()` call re-reads
 * from disk. Used by the container-config generator after it writes new
 * `allowedRoots` entries (otherwise the cache predates the write and the
 * generator's mounts get rejected at spawn time).
 */
...
/**
 * Idempotently add an allowedRoot to the allowlist file. Creates the file
 * (and parent dirs) if missing. Returns `{ added: true }` when the root
 * was newly inserted, `{ added: false }` when an entry with the same
 * literal path already existed.
 *
 * After a successful write the in-memory cache is invalidated so subsequent
 * `validateMount` calls observe the new entry.
 *
 * The caller is responsible for passing an already-resolved absolute path
 * — this helper does no path resolution of its own (matching `loadMountAllowlist`
 * which does that lazily inside `findAllowedRoot`).
 */
export function addAllowlistEntry(root: AllowedRoot): { added: boolean } {
  let allowlist: MountAllowlist = { allowedRoots: [], blockedPatterns: [] };
  if (fs.existsSync(MOUNT_ALLOWLIST_PATH)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(MOUNT_ALLOWLIST_PATH, 'utf-8')) as Partial<MountAllowlist>;
      allowlist = {
        allowedRoots: Array.isArray(parsed.allowedRoots) ? parsed.allowedRoots : [],
        blockedPatterns: Array.isArray(parsed.blockedPatterns) ? parsed.blockedPatterns : [],
      };
    } catch (err) {
      log.warn('Existing mount-allowlist failed to parse; overwriting with a fresh file', {
  ...
  const existing = allowlist.allowedRoots.find((r) => r.path === root.path);
  if (existing) {
    return { added: false };
  }
