...
import { regenerateForGroup } from './container-config-generator.js';
...
export function initGroupFilesystem(group: AgentGroup, opts?: { instructions?: string }): void {
  ...
  // Seed mcpServers + additionalMounts from data/mcp-servers.json so new
  // groups inherit operator defaults. Skipped for pipeline-* synthetic
  // groups (pipeline-loader owns those).
  if (!group.folder.startsWith('pipeline-')) {
    regenerateForGroup(group.folder);
  }
