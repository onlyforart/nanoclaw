...
/**
 * Access-gate extension hook. Fork plugins (loaded via §4.6.5 plugin
 * loader) register extensions that run BEFORE the base gate.
 *
 * Each extension returns `AccessGateResult` to short-circuit the
 * decision (`{ allowed: true }` to bypass, `{ allowed: false, reason }`
 * to deny) or `null` to fall through to the next extension and
 * ultimately the base gate. Extensions returning `{ allowed: false }`
 * are responsible for recording their own `dropped_messages` row —
 * the base gate's audit code is short-circuited.
 */
export type AccessGateExtensionFn = (
  event: InboundEvent,
  userId: string | null,
  mg: MessagingGroup,
  agentGroupId: string,
...
const gateExtensions: AccessGateExtensionFn[] = [];

export function registerAccessGateExtension(fn: AccessGateExtensionFn): void {
  gateExtensions.push(fn);
}

/**
 * Pure composition of access-gate extensions over a base gate. Run
 * extensions in registration order; first non-null result wins. If no
 * extension claims the decision, fall through to `baseGate`. If no
 * base gate is registered either, default-allow (matches the
 * pre-extension trunk behaviour when the permissions module is
 * absent).
 *
 * Exported for unit testing — production calls inline at the
 * gate-call site in `routeInbound`.
 */
export function composeAccessGate(
  extensions: AccessGateExtensionFn[],
  baseGate: AccessGateFn | null,
  event: InboundEvent,
  userId: string | null,
  mg: MessagingGroup,
  agentGroupId: string,
  ...
  for (const ext of extensions) {
    const r = ext(event, userId, mg, agentGroupId);
    if (r !== null) return r;
  }
  if (!baseGate) return { allowed: true };
  return baseGate(event, userId, mg, agentGroupId);
}
...
export async function routeInbound(event: InboundEvent): Promise<void> {
  ...
  for (const agent of agents) {
    ...
    const accessOk =
      engages && composeAccessGate(gateExtensions, accessGate, event, userId, mg, agent.agent_group_id).allowed;
