...
CREATE TABLE IF NOT EXISTS messages_in (
  ...
  channel_type   TEXT,
  thread_id      TEXT,
  -- Per-message engine override. NULL = use the container's default provider
  -- (from container.json). Non-NULL = use engine-selector to route. Examples:
  -- "claude:haiku", "ollama:qwen3", "ollama-remote:qwen3", "anthropic:haiku".
  -- Used by scheduled tasks to pick a different engine per task (v1 parity).
  model          TEXT,
  -- Per-message override for "scheduled task should use the Agent SDK". When
  -- the message is a scheduled task and use_agent_sdk = 0 (default behaviour),
  -- engine-selector routes through anthropic-api to skip the SDK preset.
  -- When 1, the SDK preset is used. NULL = default-falsy (treat as 0).
