...
export function insertMessage(
  ...
  message: {
     ...
     */
    trigger?: 0 | 1;
    /** Per-message engine override (e.g. "ollama:qwen3"). NULL = container default. */
    model?: string | null;
    /** Per-message SDK override for scheduled tasks. NULL/0 = anthropic-api; 1 = SDK. */
    useAgentSdk?: 0 | 1 | null;
  },
): void {
  db.prepare(
    `INSERT INTO messages_in (id, seq, kind, timestamp, status, platform_id, channel_type, thread_id, content, process_after, recurrence, series_id, trigger, model, use_agent_sdk)
     VALUES (@id, @seq, @kind, @timestamp, 'pending', @platformId, @channelType, @threadId, @content, @processAfter, @recurrence, @id, @trigger, @model, @useAgentSdk)`,
  ).run({
    ...message,
    trigger: message.trigger ?? 1,
    model: message.model ?? null,
    useAgentSdk: message.useAgentSdk ?? null,
    seq: nextEvenSeq(db),
  });
}
...
export function migrateMessagesInTable(db: Database.Database): void {
  ...
  if (!cols.has('model')) {
    // Per-message engine override (engine-selector input). NULL = use the
    // container's default provider. No backfill — pre-existing rows don't
    // have an opinion on engine, leaving them NULL preserves the old
    // "everything goes through the default" behaviour.
    db.prepare('ALTER TABLE messages_in ADD COLUMN model TEXT').run();
  }
  if (!cols.has('use_agent_sdk')) {
    // Per-message override for "scheduled task should use the SDK". NULL
    // (and 0) routes scheduled tasks through anthropic-api to skip the SDK
    // preset; 1 forces the SDK path. No backfill — NULL is the desired
    // default for legacy rows so they continue routing through the
    // container default with no engine override.
    db.prepare('ALTER TABLE messages_in ADD COLUMN use_agent_sdk INTEGER').run();
  }
}
