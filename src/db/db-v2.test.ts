...
// ── Messaging Group Agents — wiring settings (K.1.f step 9.0) ──
//
// Six per-wiring columns ported from v1's `registered_groups`:
//   is_main          INTEGER NOT NULL DEFAULT 0  (per-wiring "main" marker;
//                                                  at-most-one is_main=1 per
//                                                  agent_group_id, enforced
//                                                  by partial unique index)
//   model            TEXT     nullable           (engine override, NULL falls
//                                                  back to backend-defaults)
//   temperature      REAL     nullable
//   max_tool_rounds  INTEGER  nullable
//   timeout_ms       INTEGER  nullable
//   show_thinking    INTEGER  nullable           (0/1)
//
// Reauthor target: webui's "edit group" UX (per Q7 β agent-group-primary)
// reads + writes these on the wiring. Container-runner reads them at
// session spawn and injects via env vars (`NANOCLAW_MODEL` etc.).

describe('messaging group agents — wiring settings', () => {
  beforeEach(() => {
    createAgentGroup({
      id: 'ag-1',
      name: 'Agent',
  ...
  const mga = (overrides: Partial<Parameters<typeof createMessagingGroupAgent>[0]> = {}) => ({
    id: 'mga-1',
    messaging_group_id: 'mg-1',
    agent_group_id: 'ag-1',
    engage_mode: 'pattern' as const,
  ...
  it('WS1 — persists explicit wiring settings on create', () => {
    createMessagingGroupAgent(
      mga({
        is_main: 1,
    ...
    const row = getMessagingGroupAgent('mga-1')!;
    expect(row.is_main).toBe(1);
    expect(row.model).toBe('claude-haiku-4.5');
    expect(row.temperature).toBe(0.4);
    expect(row.max_tool_rounds).toBe(100);
    expect(row.timeout_ms).toBe(600_000);
    expect(row.show_thinking).toBe(1);
  });

  it('WS2 — defaults is_main=0 and other settings NULL when not provided', () => {
    createMessagingGroupAgent(mga());
    const row = getMessagingGroupAgent('mga-1')!;
    ...
    expect(row.show_thinking).toBeNull();
  });

  it('WS3 — partial unique index blocks two is_main=1 wirings for the same agent_group_id', () => {
    createMessagingGroupAgent(mga({ id: 'mga-1', messaging_group_id: 'mg-1', is_main: 1 }));
    expect(() => createMessagingGroupAgent(mga({ id: 'mga-2', messaging_group_id: 'mg-2', is_main: 1 }))).toThrow();
  });

  it('WS4 — allows multiple is_main=0 wirings for the same agent_group_id', () => {
    createMessagingGroupAgent(mga({ id: 'mga-1', messaging_group_id: 'mg-1' }));
    expect(() => createMessagingGroupAgent(mga({ id: 'mga-2', messaging_group_id: 'mg-2' }))).not.toThrow();
    expect(getMessagingGroupAgents('mg-1')).toHaveLength(1);
    expect(getMessagingGroupAgents('mg-2')).toHaveLength(1);
  });

  it('WS5 — partial unique index allows one is_main=1 per agent_group_id (different agents can each have their own main)', () => {
    createMessagingGroupAgent(mga({ id: 'mga-1', messaging_group_id: 'mg-1', agent_group_id: 'ag-1', is_main: 1 }));
    expect(() =>
      createMessagingGroupAgent(mga({ id: 'mga-2', messaging_group_id: 'mg-2', agent_group_id: 'ag-2', is_main: 1 })),
    ).not.toThrow();
  });

  it('WS6 — updateMessagingGroupAgent persists changes to all 6 fields', () => {
    createMessagingGroupAgent(mga());
    ...
    const row = getMessagingGroupAgent('mga-1')!;
    expect(row.is_main).toBe(1);
    expect(row.model).toBe('ollama:llama3.1');
    expect(row.temperature).toBe(0.7);
    expect(row.max_tool_rounds).toBe(50);
    expect(row.timeout_ms).toBe(300_000);
    expect(row.show_thinking).toBe(0);
  });

  it('WS7 — updateMessagingGroupAgent can clear settings to NULL (model/temp/etc.)', () => {
    createMessagingGroupAgent(mga({ model: 'claude-haiku-4.5', temperature: 0.4 }));
    updateMessagingGroupAgent('mga-1', { model: null, temperature: null });
    const row = getMessagingGroupAgent('mga-1')!;
    expect(row.model).toBeNull();
    expect(row.temperature).toBeNull();
  });

  it('WS8 — updating is_main from 1 → 0 frees the slot for another wiring', () => {
    createMessagingGroupAgent(mga({ id: 'mga-1', messaging_group_id: 'mg-1', is_main: 1 }));
    updateMessagingGroupAgent('mga-1', { is_main: 0 });
    expect(() => createMessagingGroupAgent(mga({ id: 'mga-2', messaging_group_id: 'mg-2', is_main: 1 }))).not.toThrow();
  });
});
