...
import { migration014 } from './014-wiring-agent-settings.js';
import { migration015 } from './015-container-task-run-logs.js';
...
/**
 * Migrations registered at runtime by fork-loaded plugins (plugin
 * loader, §4.6.5 of the v1→v2 migration plan). Ordered AFTER the
 * static `migrations` array so plugin migrations may FK-reference
 * core tables.
 *
 * v2 trunk modules use the static array above (e.g.
 * `moduleAgentToAgentDestinations`); this registry is for fork
 * plugins only — they load at runtime and cannot statically import
 * into core's array.
 */
...
export function registerMigration(m: Migration): void {
  if (registeredNames.has(m.name)) {
    log.warn('Migration already registered, skipping duplicate', { name: m.name });
    return;
  }
  registeredNames.add(m.name);
  registeredMigrations.push(m);
}
...
export function runMigrations(db: Database.Database): void {
  ...
  const all = [...migrations, ...registeredMigrations];
