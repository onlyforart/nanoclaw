...
export function createMessagingGroupAgent(mga: MessagingGroupAgent): void {
  // Wiring-settings columns (is_main / model / temperature / max_tool_rounds /
  // timeout_ms / show_thinking) ported from v1's registered_groups in K.1.f
  // step 9.0. is_main defaults to 0; the others default to NULL → fall back
  // to backend-defaults at session spawn. The partial unique index on
  // is_main=1 (migration 014) enforces "at most one main wiring per
  // agent_group_id" race-safely.
  getDb()
    .prepare(
    ...
    .run({
      ...mga,
      is_main: mga.is_main ?? 0,
      model: mga.model ?? null,
      temperature: mga.temperature ?? null,
      max_tool_rounds: mga.max_tool_rounds ?? null,
      timeout_ms: mga.timeout_ms ?? null,
      show_thinking: mga.show_thinking ?? null,
    });
