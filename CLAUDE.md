...
### Agent secret mode (OneCLI 1.7+ default = `all`)

When the host first spawns a session for a new agent group, `container-runner.ts:385` calls `onecli.ensureAgent({ name, identifier })`. The OneCLI `POST /api/agents` endpoint on **1.7 and later** creates the agent in **`secretMode: all`** by default — every vault secret with a matching host pattern is available to that agent from first spawn. No manual flip is needed.

If you want a narrower per-agent scope (e.g. an agent that should only see one specific Anthropic key, not every credential the host pattern would otherwise match), use the commands below to switch to `selective` mode and assign individual secrets.

Older `onecli` versions defaulted to `selective`. If you observe `401 Unauthorized` from `api.anthropic.com` on a new agent's first spawn, `onecli agents list` will show the agent's current mode — flip to `all` (or assign specific secrets) if you're on a pre-1.7 release.

The SDK does not expose `setSecretMode` — the per-agent flip is via the CLI or the web UI at `http://127.0.0.1:10254`.
